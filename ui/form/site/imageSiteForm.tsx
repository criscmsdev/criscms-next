'use client'
import { useUpdateImageSiteMarketing } from '@/hooks/marketing/sites/useUpdateSiteImage';
...
export const ImageSiteForm: FC<ImageSiteForm> = ({ toggle, setLeft, site, image }) => {
  ...
  const { register, handleSubmit, formState: { errors }, getValues, setValue, watch } = useForm<FormValues>({ defaultValues: { logo:  {src: site?.dataSite.imageSite?.logo?.src} , site: { src: site?.dataSite.imageSite?.banner?.src }, icon: { src: site?.dataSite.imageSite?.icon?.src } } });

  const { mutate: updateImageSiteMarketing } = useUpdateImageSiteMarketing()
  ...
  const onFileSelected = async ({ target }: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      for (const file of target.files) {
        ...
        const { data } = await axios.post(`http://localhost:6002/upload/file`, result, {
          headers: {
        ...
        setValue('input', { src: data.url, alt: `description image of the ${site?.dataSite.name}` }, { shouldValidate: true })
        updateImageSiteMarketing({ id: site?.id!, images: getValues('input'), type: type, uid: session?.user.sid! })
        if (type === 'logo') {
          if (getValues('logo')) {
            let logo = getValues('logo').src.split('/').at(-1)?.split('.').at(-2)
            await axios.post(`${process.env.API_URL}/upload/delete`, {name: logo, type: "site"} )
          }
          setValue('logo', { src: data.url, alt: `description image of the ${site?.dataSite.name}` }, { shouldValidate: true })
        } else if (type === "banner") {
          ...
          setValue('site', { src: data.url, alt: `description image of the ${site?.dataSite.name}` }, { shouldValidate: true })
        } else {
          ...
          setValue('icon', { src: data.url, alt: `description image of the ${site?.dataSite.name}` }, { shouldValidate: true })
          ...
          <div className="bg-white px-4 py-5 sm:p-6">
            ...
            <div className='pb-3'>
              ...
              <div className="mt-1 flex items-center">
                ...
                <input
                  name="file" accept=".png, .jpeg, .jpg, .webp" type="file"
                  hidden
                  // onChange={(event) => setFiles((event?.target?.files || null) as File[] | null)}
